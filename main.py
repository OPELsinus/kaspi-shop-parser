...
def performer(web, main_url, task_id):
    ...
    with suppress(Exception):
        ...
        web.find_element("//li[contains(@class, 'select__list-item') and text()='Высокий рейтинг']").click()
    # web.find_element('//*[@id="scroll-to"]/div[3]/div/div/div[2]/div').click()
        ...
        while True:
            ...
            try:
                ...
                for item in items:
                    ...
                    if reviews_count > 0:
                        if dick.copy() not in all_items:
                            # print(all_items, dick.copy(), sep='\n', end='\n---------------------')
                ...
                if 'disabled' in class_ or not web.wait_element('//*[@id="scroll-to"]/div[4]/div[2]'):
                    break
    ...
    for ind_, item in enumerate(all_items):
        ...
        for tries in range(5):
            try:
                print(datetime.datetime.now(), '|', ind_, item)
                ...
                lowest_price_for_item = 10000000
                ...
                if web.wait_element("//li[contains(text(), 'Характеристики')]", timeout=0.3):

                    web.execute_script_click_xpath_selector("//li[contains(text(), 'Характеристики')]")

                    if web.wait_element("//span[contains(text(), 'Вес')]", timeout=0.3):

                        weight = web.find_element("//span[contains(text(), 'Вес')]/../following-sibling::dd").get_attr('text')
                ...
                df.loc[len(df)] = {'title': item['title'], 'link': item['link'], 'subcategory': subcategory_name, 'item_code': item_code, 'rassrochka': rassrochka,
                                   ...
                                   'reviews_last_month': total_feedbacks_last_month, 'weight': weight, 'price': lowest_price_for_item, 'sellers_info': all_sellers_off_cur_item}
                df.to_excel('sdfsdlfk.xlsx')
                if len(df) % 10 == 0:
                    df_ = df.copy()
